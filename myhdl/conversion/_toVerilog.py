...
_translate_on = re.compile(r"\s*(synthesis|synopsys)\s+translate_on")
_translate_off = re.compile(r"\s*(synthesis|synopsys)\s+translate_off")
...
def _makeDoc(doc, indent=''):
    ...
    if _translate_off.match(doc):
        doc = "`ifndef SYNTHESIS"
    elif _translate_on.match(doc):
        doc = "`endif"
    else:
        pre = '\n' + indent + '// '
        doc = '// ' + doc
...
def _debug_str(s):
    if s._debug_level is not None:
        return f"  // dbglvl:{s._debug_level} dbgdesc:{s._debug_descr}"
    else:
        return ""
    ...
    for portname in intf.argnames:
        ...
        if s._driven:
            ...
            if isinstance(s, _TristateSignal):
                ...
                print(
                    "output %s%s%s;%s" % (p, r, portname, _debug_str(s)),
                    file=f,
                )
            if s._driven == 'reg':
            ...
            print("input %s%s%s;%s" % (p, r, portname, _debug_str(s)), file=f)
    print(file=f)
    ...
    for s in siglist:
        ...
        if s._driven:
            ...
            if not toVerilog.initial_values or k == 'wire':
                print(
                    "%s %s%s%s;%s" % (k, p, r, s._name, _debug_str(s)), file=f
                )
            else:
            ...
            constwires.append(s)
            print("wire %s%s;%s" % (r, s._name, _debug_str(s)), file=f)
    # print(file=f)
    for m in memlist:
        if not m._used:
            continue
        ...
        print("%s %s%s%s [0:%s-1];%s" % (k, p, r, m.name, m.depth, dbg),
              file=f)
...
_translate = re.compile(r"\s*(synthesis|synopsys)\s+translate.*")
...
class _ConvertVisitor(ast.NodeVisitor, _ConversionMixin):
    ...
    def writePreFuncDoc(self, node):
        assert hasattr(node, "doc")
        if node.doc and _translate.match(node.doc):
            pass
        else:
    ...
    def writePostFuncDoc(self, node):
        assert hasattr(node, "doc")
        if node.doc and _translate.match(node.doc):
            doc = _makeDoc(node.doc, self.ind)
            self.writeline()
    ...
    def writeAlwaysHeader(self):
        ...
        always = "always"
        has_senslist = True
        if len(senslist) in (1, 2):
            clocked = isinstance(senslist[0], (
                    myhdl._Signal._PosedgeWaiterList,
                    myhdl._Signal._NegedgeWaiterList
                )
            )
            resetted = clocked and len(senslist) == 2
        else:
            clocked = False
            resetted = False
        if toVerilog.standard == "systemverilog":
            if clocked:
                always = "always_ff"
            else:
                always = "always_comb"
...
#        self.writeline()
    ...
    def getName(self, node):
        ...
        if n == "False":
            ...
            if isinstance(obj, bool):
                ...
                s = n
            elif self.context == _context.PRINT and isinstance(obj, str):
                s = f'"{obj}"'
            else:
    ...
    def visit_Print(self, node):
        ...
        for s in node.format:
            ...
            self.writeline()
        if node.nl:
            self.write('$write(" ", $time, "ns. RTL_INFO!\\n");')
